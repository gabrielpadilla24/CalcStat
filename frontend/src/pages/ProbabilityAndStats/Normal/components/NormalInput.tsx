...
export default function NormalInput({ onResult }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      const res = await api.post<NormalResponse>("/normaldistribution", params);
      onResult(res.data);
    } catch (err) {
    ...
    <div className="bg-white rounded-xl shadow-md border border-gray-200 p-4 sm:p-6 w-full flex flex-col">
      ...
      <div className="flex justify-center mt-6">
        <button
          ...
          className="bg-[#5FBA9B] text-white px-4 sm:px-6 py-2 rounded-lg font-semibold hover:bg-[#4da88a] transition w-full"
        >
